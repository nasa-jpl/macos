...
This directory also produces two further documents:

- the **MACOS Command Reference** (`cmdref/`) — a standalone catalog of
  every engine command and every mmacos/pymacos function, written to
  guide new users of the public repo.  See "Command Reference" below.
- the **Polarization Validation Report** (`polval/`) — the
  reviewer-facing evidence for the polarization work: one section per
  validation gate, with figures and measured numbers.  Unlike the other
  two, its numbers and figures are *generated by re-running the
  validation cases*.  See "Polarization Validation Report" below.
...
make polval     # build/macosPolVal.docx + .html  (polarization validation)
make polval-pdf # build/macosPolVal.pdf
make polval-regen # re-measure every validation case (needs MATLAB)
make clean
```
...
`polval/` holds the reviewer-facing companion to the polarization test
code, specified in `PLAN_POLARIZATION.md`: one evidence section per
validation gate, each stating the claim, the figure, the measured
number, the truth it is compared against, and the test that pins it in
CI.

**It is built differently from the manual and the cmdref, and the
difference matters.** Every number and every figure is generated by
actually re-running the validation cases:

- the prose sources are `polval/*.md.in` — they contain **no numeric
  literals**, only substitution placeholders;
- `polval/*.md`, `polval/media/*.png` and `polval/generated/numbers.json`
  are **generated — never hand-edit them**;
- `make polval-regen` runs the driver in
  `MACOS_resources/mmacos/tools/pol_validation_report/` (MATLAB + a
  built mmacos mex), which re-measures every case, rewrites the figures
  and the numbers, and re-renders the prose;
- `make polval` refuses to build if a template was edited without
  re-rendering, if a figure is newer than the numbers, or if any
  placeholder is unresolved (`tools/check_polval.py`).

So a stale figure cannot ship beside a fresh number.  The report also
stamps the engine and binding commits, model size and host the numbers
came from, and labels every number the driver *cannot* produce (pymacos
/ ifx, GMI, historical pre-fix engine A/Bs) as externally sourced with
its capture date.  See
[`MACOS_resources/mmacos/tools/pol_validation_report/README.md`](../../../MACOS_resources/mmacos/tools/pol_validation_report/README.md)
for the driver, and add an evidence section there as each polarization
phase lands.
