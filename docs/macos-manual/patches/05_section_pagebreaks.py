...
def transform(root):
    ...
    for p in iter_paragraphs(root):
        if get_style(p) not in PAGE_BREAK_STYLES:
            continue
        # The very first Heading2 in the document is the start of body
        # content -- adding pageBreakBefore there would push it past
        # the cover page unnecessarily.  Skip the first match; every
        # subsequent SECTION / APPENDIX gets a break.
