...
* **The out-of-plane attribution in §3.5 and §3.6 is unverified.** Where the
  vector run legitimately differs from the scalar one on off-normal trains,
  the difference is attributed to out-of-plane field content on order-of-
  magnitude grounds only. Closing it needs a plane-selectable complex-field
  getter, which does not exist. The gates bound the difference; they do not
  explain it.
