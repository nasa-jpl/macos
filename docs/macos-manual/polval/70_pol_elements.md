...
## 6.7 Scope: normal incidence, and one convention left open
...
projecting that, and transmitting the complement. MACOS declares the pass
axis. The two constructions disagree by **3.56°** of axis
orientation at 20° of incidence (at 45° of azimuth, where the effect is
largest; the closed form is `acos(2cos a/(1+cos²a))`, bounded by `sin²a`).
...
Consequently `RfPolarizer` — a *reflective* polarizer, which is inherently
off-normal — is still not implemented. It cannot be, without settling this
first. For a beamsplitter at a substantial angle, a coated `Reflector` is in
