# BRIEF (was CCMac's; items 3-8 now TO's): the bench must be buildable -- node clearance, substrates, a real camera
...
| 2 | **done by CCL** (recovered after the 07:25 OOM crash; resources 1338920): Stage A solves the angle against the node parts, the `clearance` stage prints the part table (node22t: worst +38 mm, spec 25); the OAP fold re-solve moves to TO's reflective brief |
| 3-8 | **TO's, after `BRIEF_to_reflective.md`** (Dave 2026-09-15: CCMac will not return before this work concludes).  In the order below from item 3; each whole, committed; the interferometer's snapshot polarization at 22.5 (item 6) and its station figure (item 8) included |
