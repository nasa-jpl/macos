...
- 1.3 The loop: EFC on the engine itself (hard 2.9e-7 -> 8.1e-9; vortex 1.7e-8 -> 6.8e-15); polarization (1.1e-15 residual, a state change); bandwidth (mono 8e-13 -> 20% 5.4e-11); the vector vortex verdict (leak uncorrectable but optically removable).  **These numbers are being re-scored: the DM model used the beam radius as its diameter (TO 2026-09-15), so the committed lattice covered the inner 28% of the pupil area; self-consistent on an aberration-free train, corrected before any aberration or drift work.**  Old / new table here when TO lands it.
