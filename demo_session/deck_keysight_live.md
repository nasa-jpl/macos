...
| reproduction | 0.991–1.003× | 0.95–1.02× | 0.8–1.25× (all five steps) |
...
-- **The room's answer:** give the optimizer a made-up glass whose properties vary smoothly, and it can walk into a new design form — and when real glass is swapped back in, the new form keeps.  It does not slide back.
...
-- **The room's answer:** measure every requirement against its own tolerance, so all are in the same units — then weight each one.
~ Asked and answered live, 2 September 2026.
...
-- **The room's answer:** judge the pupil image only at its edge — that is all the cold stop feels.  Accept mapping distortion to keep image sharpness (a classical trade), and fix what remains downstream in the instrument, where the beam is small.
...
-- Not answered in the room — still open.
~ Asked live 2 September 2026; the pupil questions were answered from the floor, the decenter effect remains open.
...
- **Zernike figure modes 4–11** on each of the 19 segments, M2-M3, the 8 OAPs — five fields each.  Each channel is confined to its own segment; the mode shape is visible at a glance.
...
~ Asked live 2 September 2026 — no answer relayed; open for correspondence.

## The live design, revealed | Predicted from the frontier before solving; solved in one warm-started step while we talked
::: left
- **The ask from the room:** a 12×12° field box.  **The prediction, stated before the solve:** ~33.7 nm, interpolated from the committed frontier rows.
- **The live run [PASS]:** dense-map max **33.6 nm** — 1.00× the prediction — at a 24.9 mm clearance floor, in 10.6 minutes, while we talked.
...
::: right
![The room's 12° design and its field map: 33.6 nm max against a ~33.7 nm prediction.](figs/oi_demo_live_layout.png){h=3.0}
![](figs/oi_demo_live_map.png){h=2.2}
~ Solved live during this talk; artifacts oi_demo_12deg_20260902_102214_* in the demo_adjacent directory.
...
~ These are the questions this work earned; the next slides are what we would recommend.  (Asked 2 September 2026; open for correspondence.)
