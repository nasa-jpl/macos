...
- [ ] **Parser: preserve WHOLE-LINE comments through SAVE (not inline comments).**  Added 2026-07-01 (Dave).  So a load→SAVE round-trip keeps documentation comment LINES; deliberately NOT inline (`key= value % comment`) comments, which are the fragile case that produced the GridFile-tab bug above.
