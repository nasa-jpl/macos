...
- [ ] **Bake comprehensive default glass tables into macos/smacos so no separate runtime glass-table file is needed.**  Added 2026-07-01 (Dave).  Today `macos_glass.inc` reads `glassFile` at startup and falls back to `Air` only when absent; ship the full catalog compiled in.
- [ ] **`opd` / the OPD path SIGSEGVs when all rays are obscured/lost (`nValidRays==0`).**  Found 2026-07-01 (Dave, command-line: `opd 55` after every ray is obscured at elt 1 on `optiix_dp_conic` → `forrtl: severe (174) SIGSEGV`; it also killed the mmacos OPD test).  Debug build can't reach it — the `-fcheck=bounds` build trips first on the legacy assumed-size idiom `REAL*8 A(1); A(1:N)=0` in `DZERO`/`IZERO` (`mathsub.F`, a FALSE positive; `A(1)`→`A(*)` would silence it).  Harden the OPD/SrfOut path to report "all rays lost" gracefully instead of segfaulting.
