...
- [x] **Parser: preserve WHOLE-LINE comments through SAVE (not inline comments).**  DONE 2026-07-03 (macos sls-dev).  Whole-line `%` comments are captured at load in `GET_EQ`'s comment branch (`RxCommentCapture`, gated by `LRxCommentCapture` so the MOD dialog — which also calls GET_EQ — never captures) into `elt_mod` storage (`RxCommentLine/Anchor`, 500-line cap), anchored to the element context at capture (0 = header, iElt = that element's block; `RxCommentCtx` maintained by MBFile6), and re-emitted by SAVE (`RxCommentEmit` at `PrtSourceInfo` end + `PrtSingleEltInfo` end — a comment preceding `iElt= k+1` is read during block k, so end-of-block-k emission reproduces its position).  SAVE's own banner furniture (dashes-only lines + the three section titles) is filtered at capture so load(SAVE'd)→SAVE stays idempotent.  Inline comments deliberately NOT preserved per spec.  Verified on eac2_7seg (14 comment lines incl. commented-out keyword lines survive at correct positions; pass2==pass3 except a pre-existing 2-ulp `ChfRayPos` re-aim oscillation under ApStop).  Rode along: `FmtD` now prints **17 sig digits** (was 16 — not always bit-exact on reload) and the unset-units placeholder `BaseUnits/WaveUnits= (default-unit)` is no longer written (GET_EQ eats parens on reload → the value came back empty; was a second idempotency breaker).  ORIGINAL NOTE — Added 2026-07-01 (Dave): so a load→SAVE round-trip keeps documentation comment LINES; deliberately NOT inline (`key= value % comment`) comments, which are the fragile case that produced the GridFile-tab bug above.
- [x] **Bake comprehensive default glass tables into macos/smacos so no separate runtime glass-table file is needed.**  DONE 2026-07-03 (macos sls-dev).  `tools/gen_glass_builtin.py` generates `macos_f90/glass_builtin.f90` (`SUBROUTINE LoadBuiltinGlass`) from the canonical 200-glass `macos_f90/macos_glass_list.txt` (Sellmeier, 6 coefs; all four copies on disk were identical; mGlass=250 accommodates).  All THREE loader fallbacks — `macos_glass.inc` (macos startup), `smacos_glass.inc` (SMACOS init), `rl_macos_glass.inc` (the GLASS reload command) — now call it instead of the hardcoded Air/BK7/LAK9 trio; a `macos_glass_list.txt` found at runtime still OVERRIDES (search order unchanged: CWD → exe dir → $MACOS_HOME → BUILD_LOC).  Generated table verified programmatically identical to the txt (all 200×6 values + names + order).  Note: NO existing Rx in any corpus uses `Glass=` (all use explicit IndRef) — glass-by-name refraction has zero test coverage; a `Glass= BK7` fixture would be a worthwhile follow-on.  ORIGINAL NOTE — Added 2026-07-01 (Dave).
- [x] **`opd` / the OPD path SIGSEGVs when all rays are obscured/lost (`nValidRays==0`).**  FIXED 2026-07-03 (macos sls-dev).  Root cause (gdb on the ifx CLI, pty-driven): **`DXCALC` (utilsub.F) — when EVERY pupil slice has zero valid rays, the slice-search loop never assigns `iimax`** (only set when `l>lmax`, and all `l` stay 0), and the second pass indexes `LRayOK` from `j=(iimax-1)*npts+1` built on uninitialized garbage.  ifx stack garbage → wild index → `forrtl severe(174)`; gfortran's garbage happened benign, which is why it looked ifx-only.  The pre-existing `dxRef=1d0` l==0 guard was necessary but not sufficient.  Fix: init `iimax=1` + skip the second slice pass when `lmax==0` (the existing "grid spacing failed" warning + degenerate-beam guards then handle it).  Verified: ifx CLI `OPD 55` on all-obscured segmented Rx (bad-ApStop variant) prints "All rays are obscured or lost" + the 9.9999e36 sentinel and returns to the prompt; mmacos `opd` on the same Rx leaves MATLAB alive (its wrapper returns the sentinel non-struct — optional veneer polish).  mmacos 30 classes / 0 failures; GMI all-pass.  ORIGINAL NOTE — Found 2026-07-01 (Dave, command-line: `opd 55` after every ray is obscured at elt 1 on `seg_dp_conic`; it also killed the mmacos OPD test).  The `DZERO`/`IZERO` `A(1)`→`A(*)` false-positive blocker was separately fixed (`31fad6b`).
- [x] **GridData (SrfType-9) grid surfaces sampled only the center pixel (pure piston).**  Closed 2026-06-29 (sls-dev `03db580`, cherry-picked opt-dev `1b535a5`).  `GridSrf` (the SrfType-9 thin wrapper over `FreeFormSrf`, surfsub.F) forwarded an all-zero grid coordinate frame, collapsing FreeFormSrf's grid-index calc (`xi=xData·rhom/dAct`, `yj=yData·rhom/dAct`) to the center pixel for every ray — so a GridData grid acted as a pure piston and its spatial figure was discarded (a regression from the FreeForm refactor when `GridSrf` became the wrapper; `MonGridSrf`/`FreeFormSrf` always forwarded the real frame).  Symptom: a GridData segment grid-figure poke imaged as a whole-pupil per-segment piston (the SegDemo3conic dw/dgrid example).  Fix: thread the element's real `pData/xData/yData/zData` through `GridSrf` + its FindSrf/Reflector/Refractor call sites in elemsub.F.  GMI regression 6/6 bit-identical (its Rx use FreeForm/Conic, not SrfType-9).  Deferred: `NSRefractor` (non-sequential) still null-frames GridData.  See `macos_f90/CLAUDE.md` "GridSrf (SrfType-9)" section.
- [x] **NS follow-on: grid figures on NSReflector segments still collapsed to a center-pixel per-segment piston.**  Closed 2026-07-02 (sls-dev `60f886d`; **opt-dev cherry-pick pending** — it has the same slip via `1b535a5`).  The frame-threading fix above used `iElt` at the two NON-sequential Reflector call sites (tracesub.F:3714, propsub.F:983) where the intersected element is `imin`; `iElt` there is the frameless NS-group entry (zeros → every ray → GridMat center pixel).  Caught by the `ns_griddata` 5-variant decomposition (MACOS_resources `e6f4f17`, Luis's iris_dp case), which also documents `GridSrfdx` span semantics (`(nGridMat−1)·dx`, centered on `pData`; 280/255 ≈ 1.1 for the iris segments).  mmacos regression 30 classes / 0 failures.  Remaining: the `NSRefractor` ROUTINE (refractive NS) still null-frames GridData.
- [x] **pymacos `spot()` passed `ref_csys`/`ref_pos` swapped.**  Closed 2026-06-15 (MACOS_resources sls-dev `2193705`).  `macos.py`'s `spot` forwarded `vpt_center → ref_csys` and `beam_csys → ref_pos` (reversed vs `spot_cmd`'s signature), so `spot(...,vpt_center=False)` sent `ref_csys=0` (an invalid frame) and `spot_cmd` rejected EVERY call — `m.spot` was broken for all Rx.  No pymacos spot test existed to catch it.  Fix: `beam_csys → ref_csys`, `vpt_center → ref_pos`; validated against mmacos (the `obs_set('all')`+spot coma-broadening result matches to 234.8×).
...
- [ ] Prescription output formatter missing
  ...
  - [ ] Regression: round-trip `e2e_pie_met.in` and confirm METcalc output identical
...
- [ ] GMI regression coverage
  - [ ] `e2e_pie_met.in` already exercises METcalc but isn't in the GMI regression suite — add it (alongside §1.1 GMI test expansion)
...
- [x] Build `mmacos_mex.F` MATLAB bridge over `macos_api_mod.F90`
  ...
  - [x] Additional commands implemented as needs surfaced: `sxp`, `xp_get/set/fnd`, `spot_cmd/get`, `ray_info_get/set`, `stop_info_*`, `ors_run`, `srs_run`, source-side `set_src_*` / `get_src_*`, the `elt_*` Rx-inspection / -modification family; `compose_*` + `set_src_flux` (broadband PSF assembly, §`project_macos_compose_broadband`); `sysprop` / `first_order_properties` (first-order optical properties).
  - [x] **Image-placement wrappers (2026-06-15, sls-dev `5a39339` engine + `2193705`/`eb6b508` bindings):** `window_set`/`window_off` (WINDOW pixel-location option — place each source at its TRUE chief-ray offset, not re-centred), `ffp`/`pfp` (off-axis field point by direction cosine / focal-plane pixel — the two differ by the plate scale), `obs_set` (OBS ray-trace obscuration option for spot diagrams: ALL/POSITIVE/NEGATIVE — ALL is needed to draw a spot at a coronagraph FP where every ray is obscured).  In `macos_api_mod`, wrapped in pymacos (`macos.py`) + mmacos (`+macos/`).  Exercised end-to-end by `mmacos/examples/coro_planet_demo/coro_planet_demo.m` (the planet-injection redo: off-axis planet placement, on-par-with-residual scaling, obs_set spot diagram showing the 235× coma broadening = field aberration).
    - [ ] **NOT yet on opt-dev** — opt-dev lacks the COMPOSE (`6cd1372`) + SYSPROP (`927eee9`) prerequisites this sits on.  Land at the next `sls-dev → opt-dev` promotion (bring the prereqs along).
...
- [ ] Segmented-PM single-field optimization
- [ ] Segmented-PM multi-field optimization
...
`e2e_pie_init.m`, and the `test_ff/` driver scripts on 2026-05-31.
