...
- [x] **`GridFile=` names kept the trailing TAB(s) from an Rx inline comment → `GridInit` "does not exist" → the grid figure was silently dropped.**  Fixed 2026-07-01 (reported by Luis via Dave; `optiix_dp_zgd` / `iris_dp_ZGD` in `MACOS_resources/mmacos/examples/ns_griddata`).  A line like `GridFile=  zern41em5z155em3.txt<TAB><TAB>% flat.txt` parses to `VALUE = "zern41em5z155em3.txt<TAB><TAB>"` (the `%` comment is stripped, the tabs before it are not); `ICLEN` counts a tab as non-blank, so `GridInit` tried to open `zern41...txt<TAB><TAB>` → file-not-found → `GridMat=0` → no figure.  Shared parser, so GMI/mmacos/command-line all hit it (Luis saw it in GMI+mmacos on a tab-laden Rx; a clean/SAVE'd Rx works).  Proven on the SAVE'd `optiix_dp_zgd.in`: clean → "Reading surface grid data file …"; re-tabbed → "does not exist".  Fix: strip trailing blanks+TABS from the GridFile value at parse (`msmacosio.inc:1959`) + harden `GridInit` (`surfsub.F`, before the `INQUIRE`).  Verified: the tab-laden Rx now reads the grid.  Ship: sls-dev + cherry-pick opt-dev.
